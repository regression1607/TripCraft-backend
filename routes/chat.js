...
// POST /api/chat/nearby/ping - Update my location & mark as nearby-active
router.post('/nearby/ping', async (req, res) => {
  try {
    const user = await getUserId(req);
    if (!user) return res.status(404).json({ error: 'User not found' });
...
// GET /api/chat/nearby/users - Find nearby active users within ~500m
router.get('/nearby/users', async (req, res) => {
  try {
    const user = await getUserId(req);
    if (!user) return res.status(404).json({ error: 'User not found' });
    if (!user.lastLocation?.lat) return res.json({ users: [] });

    const myLat = user.lastLocation.lat;
    const myLng = user.lastLocation.lng;

    // Find users who pinged in last 5 minutes and are nearby-active
    ...
    }).select('name username avatar lastLocation').lean();

    // Filter by distance (~500m radius)
    const withinRange = nearbyUsers.filter(u => {
      const dist = haversineDistance(myLat, myLng, u.lastLocation.lat, u.lastLocation.lng);
      u.distance = Math.round(dist);
      return dist <= 500; // meters
    }).sort((a, b) => a.distance - b.distance);

    res.json({ users: withinRange });
  } catch (error) {
    res.status(500).json({ error: 'Failed to find nearby users' });
  }
});

// POST /api/chat/nearby/stop - Mark as no longer nearby-active
router.post('/nearby/stop', async (req, res) => {
  try {
    const user = await getUserId(req);
    if (user) {
      user.isNearbyActive = false;
      await user.save();
    }
    res.json({ message: 'Stopped nearby' });
  } catch (error) {
